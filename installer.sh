...
ansible ocp -a "subscription-manager clean" -u $USER
...
ansible ocp -a "subscription-manager refresh" -u $USER
ansible ocp -a "subscription-manager attach --pool=$OCPPOOLID" -u $USER
ansible ocp -a "subscription-manager repos --disable='*' --enable='rhel-7-server-extras-rpms' --enable='rhel-7-server-optional-rpms'" -u $USER
ansible ocp -a "subscription-manager repos --enable="rhel-7-server-rpms" --enable="rhel-7-server-extras-rpms" --enable="rhel-7-server-ose-3.11-rpms" --enable="rhel-7-server-ansible-2.6-rpms"" -u $USER
ansible ocp -a "yum -y install wget git net-tools bind-utils iptables-services bridge-utils bash-completion kexec-tools sos psacct" -u $USER
ansible ocp -a "yum -y update" -u $USER
ansible ocp -a "yum install -y docker-1.13.1-75.git8633870.el7_5.x86_64" -u $USER
ansible ocp -a "sed -i 's/SELINUX=disabled/SELINUX=enforcing/g' /etc/selinux/config" -u $USER
ansible ocp -a "sed -i 's/DNS1=.*/DNS1='"$DNSIP"'/g' /etc/sysconfig/network-scripts/ifcfg-$NWINTF" -u $USER
ansible ocp -a "systemctl restart network" -u $USER

#sed -i 's/SELINUX=disabled/SELINUX=enforcing/g' /etc/selinux/config

ansible-playbook ~/ocp311/nodes.yml --user=$USER
ansible ocp -a "sed -i 's/DOMAIN/.$DOMAIN/g' /etc/environment" -u $USER
ansible ocp -a "sed -i 's/DNSIP/$DNSIP/g' /etc/resolv.conf" -u $USER
ansible ocp -a "touch /.autorelabel" -u $USER
ansible ocp -a "reboot" -u $USER

sleep 2m
ansible-playbook -i /root/ocp311/inventory1 /usr/share/ansible/openshift-ansible/playbooks/prerequisites.yml
ansible-playbook -i /root/ocp311/inventory1 /usr/share/ansible/openshift-ansible/playbooks/deploy_cluster.yml